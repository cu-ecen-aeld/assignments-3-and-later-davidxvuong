...
if [ $? -ne 0 ]; then
    echo "Error - unable to create directory ${OUTDIR}"
    exit 1
fi
...
if [ ! -e ${OUTDIR}/linux-stable/arch/${ARCH}/boot/Image ]; then
    ...
    for c in mrproper defconfig all modules dtbs; do
        echo "Running: make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} ${c}"
        make -j4 ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} ${c}
    done
fi
